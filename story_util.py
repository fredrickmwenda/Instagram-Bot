...
def get_story_data(driver, elem, action_type, simulate=False):
        """
        ...
        if action_type == "user":
            try:
                reel_id = driver.execute_script("return window.__additionalData[Object.keys(window.__additionalData)[0]].data.graphql.user.id")
                ...
                try:
                    ...
                    print("---> Sorry, this page isn't available!\t~either "+ "link is broken or page is removed\n")
        ...
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.163 Safari/537.36", "X-Requested-With": "XMLHttpRequest"}
            ...
            if media_cnt == 0:
                ...
                if simulate is True:
                    for item in response["data"]["reels_media"][0]["items"]:
                        if item["taken_at_timestamp"] <= seen:
                            ...
                            print("  --> simulated watch reel # {}".format(index))
                            #update_activity()
                            index += 1
                            time.sleep(randint(3, 6))
                            reels_cnt += 1
                else:
                #    story_elem = driver.find_element_by_xpath(watch_story.__name__ + "_for_{}".format(action_type),"explore_stories",)
                    story_elem = driver.find_element_by_xpath("//section/main/div/header/div/div")
                ...
                for item in response["data"]["reels_media"][0]["items"]:
                    if item["taken_at_timestamp"] <= seen:
                        ...
                        if index == 1:
                            try:
                                next_elem = driver.find_element_by_xpath("/html/body/span/section/div/div/section/div[2]/button")
                            except NoSuchElementException:
                                continue
                        else:
                            try:
                                next_elem = driver.find_element_by_xpath("/html/body/span/section/div/div/section/div[2]/button[2]")
                            except NoSuchElementException:
                                continue
...
def watch_story(driver, elem, action_type, simulate=False):
    """
    ...
    if action_type == "tag":
        story_link = driver.get("https://www.instagram.com/explore/tags/{}".format(elem))

    else:
        story_link = driver.get("https://www.instagram.com/{}".format(elem))
    ...
    story_data = get_story_data(driver, elem, action_type, simulate)
    ...
    if story_data["reels_cnt"] == 0:
        ...
        print("no stories to watch  or you have already watched everything")
        return 0

    print("watched {} reels from {}: {}".format(story_data["reels_cnt"], action_type, elem.encode("utf-8")))
